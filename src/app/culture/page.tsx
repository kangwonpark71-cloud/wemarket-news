...
export default async function CulturePage({ searchParams }: PageProps) {
  ...
  const naverResult = await searchNaverNewsByDate('문화', {
    display: 20,
    start,
  })
  ...
  return (
    <CategoryPageLayout
      title="문화 실시간 뉴스"
      description="연합뉴스TV, YTN 문화 뉴스"
      total={rssResult.total}
      page={page}
      totalPages={rssResult.totalPages}
      basePath="/culture"
    >
      <NewsList articles={rssResult.articles} emptyMessage="아직 수집된 문화 뉴스가 없습니다" />
    </CategoryPageLayout>
  )
}
